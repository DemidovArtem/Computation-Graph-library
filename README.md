...
мы поступим хитрее. Операция сортировки за вас уже реализована в файле [`lib/external_sort.py`](lib/external_sort.py) путём
...
5. Выполнение задач на входных данных контролируют наши тесты. Есть два вида тестов: лёгкие, контролирующие правильность
результата, и сложные, на которых проверяется, что ваше решение работает в стриминговой манере (то есть, обладает константным
потреблением памяти). 

6. Формальным критерием сдачи является прохождение тестов (если ваше задание содержит не все тесты, мы оставляем за 
собой право не принять код на ревью).

7. Для каждой задачи сделайте тест, позволяющий быстро запустить её на входных данных из `resource`.
...
8. Задача проверяется на code review. Задача не будет зачтена, пока ревьювер не скажет, что она зачтена.
...
но только в том случае, если вы пришлете код не под дедлайн. Merge request должен быть аккуратным:
код решения должен быть в отдельной ветке, не затронуты лишние файлы, в частности не нужно коммитить
в репозиторий разархивированные ресурсы. Грязные merge request'ы смотреть не будем, поэтому сразу отведите
ветку от мастера для решения.
